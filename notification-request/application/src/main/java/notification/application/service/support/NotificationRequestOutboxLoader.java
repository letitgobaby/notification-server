...
public class NotificationRequestOutboxLoader {
    ...
    /**
     * RequestMessageOutbox를 로드하고, 해당 알림 요청이 존재하지 않거나 상태가 CANCELED인 경우
     * Outbox 메시지를 삭제합니다.
     *
    ...
    public Mono<NotificationRequest> load(RequestOutbox message) {
        log.info("Loading NotificationRequest for outbox: {}", message.getOutboxId());
        ...
        return notificationRequestRepository.findById(requestId)
                .switchIfEmpty(clearRequestMessageOutbox(message))
                .flatMap(domain -> {
                ...
                .doOnSuccess(domain -> log.info("Successfully loaded NotificationRequest: {}",
                        domain.getRequestId().value()))
                .doOnError(error -> {
                    log.error("Error loading NotificationRequest for outbox: {}", message.getOutboxId(), error);
                });
    }
