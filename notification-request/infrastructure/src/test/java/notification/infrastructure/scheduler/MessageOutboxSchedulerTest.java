...
class MessageOutboxSchedulerTest {

    @Mock
    private MessageOutboxPollingUseCase messageOutboxPollingService;

    @InjectMocks
    private MessageOutboxScheduler messageOutboxScheduler;
    ...
    void shouldLogSuccessfullyWhenPollSucceeds() {
        // Given
        when(messageOutboxPollingService.poll()).thenReturn(Mono.empty());

        // When & Then
        assertDoesNotThrow(() -> messageOutboxScheduler.poll());

        // Verify that the polling service was called
        verify(messageOutboxPollingService, times(1)).poll();
    }
    ...
    void shouldCallUseCaseWhenPollExecuted() {
        ...
        when(messageOutboxPollingService.poll()).thenReturn(expectedMono);

        // When
        messageOutboxScheduler.poll();
    ...
    void shouldLogErrorWhenExceptionOccursInPoll() {
        ...
        when(messageOutboxPollingService.poll()).thenReturn(Mono.error(exception));
    ...
    void shouldExecutePollAsynchronously() {
        ...
        when(messageOutboxPollingService.poll()).thenReturn(delayedMono);
    ...
    @Test
    @DisplayName("MessageOutboxPollingUseCase의 poll 메서드 반환값이 올바르게 처리된다")
    void shouldHandleReturnValueFromUseCasePollMethodCorrectly() {
        // Given
        Mono<Void> testMono = Mono.fromRunnable(() -> {
            // 실제로 뭔가 작업을 수행한다고 가정
            System.out.println("Message outbox polling executed");
        });

        when(messageOutboxPollingService.poll()).thenReturn(testMono);
    ...
    void shouldHaveCorrectScheduledAnnotationOnPollMethod() throws NoSuchMethodException {
        // Given
        Class<MessageOutboxScheduler> clazz = MessageOutboxScheduler.class;
    ...
    void shouldHaveComponentAnnotationOnClass() {
        ...
        // Then
        assertNotNull(componentAnnotation, "MessageOutboxScheduler 클래스에 @Component 어노테이션이 있어야 합니다");
    }
}
