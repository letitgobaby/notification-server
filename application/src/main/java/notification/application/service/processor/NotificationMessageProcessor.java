...
import notification.application.notifiation.port.inbound.NotificationScheduledMessageProcessorUseCase;
...
public class NotificationMessageProcessor implements NotificationScheduledMessageProcessorUseCase {
     ...
     * 
     * @param event NotificationMessageScheduledEvent
     * @return 처리 결과
     */
    @UnitOfWork
    @Override
    public Mono<Void> process(NotificationMessageScheduledEvent event) {
        log.info("Processing scheduled messages for notification request ID: {}",
                event.getOutboxMessage().getAggregateId());

        OutboxMessage outbox = event.getOutboxMessage();

        return notificationMessageOutboxLoader.load(outbox)
                .flatMap(message -> {
