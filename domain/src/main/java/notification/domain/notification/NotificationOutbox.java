...
public class NotificationOutbox {
    ...
    public NotificationOutbox(Long outboxId, String aggregateType, NotificationId notificationId,
        ...
        try {
            ...
            this.status = status == null ? OutboxStatus.PENDING : status; // 기본 상태는 PENDING
            this.retryAttempts = retryAttempts < 0 ? 0 : retryAttempts; // 재시도 횟수는 음수가 될 수 없음
            this.nextRetryAt = nextRetryAt == null ? Instant.now() : nextRetryAt; // 기본값은 현재 시각
            this.createdAt = createdAt;
        } catch (NullPointerException e) {
            throw new DomainValidationException("Required fields cannot be null.", e);
        }
    }

    // 메시지가 성공적으로 MQ로 전송되었을 때 호출되어 상태를 SENT로 변경합니다.
    // PENDING 또는 FAILED 상태일 때만 SENT로 변경 가능합니다.
        ...
        if (this.status == OutboxStatus.PENDING || this.status == OutboxStatus.FAILED) {
            ...
            throw new InvalidOutboxStatusException(
                    "Cannot mark a message with status "
                            + this.status + " as SENT. Only PENDING or FAILED messages can be marked as SENT.");
        }
    }

    // 메시지 전송에 실패했을 때 호출되어 상태를 FAILED로 변경하고 재시도 관련 정보를 업데이트합니다.
    ...
    // 메시지가 현재 전송 대기 중(PENDING) 상태인지 확인합니다.
